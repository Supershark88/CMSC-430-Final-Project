...
(provide prim? reserved? prims->list
         ...
         test-proc->llvm
         test-proc->llvm-error
         prim-divide-by-zero-bool)

; Toggles for various implementations

; If true, all divide by zero prims will be desugared into a prim halt
(define prim-divide-by-zero-bool #t)
    ...
    (if (> time (+ starttime 12000))
        ...
        (if (eq? status 'done-ok)
            ...
            (if (eq? status 'done-error)
                (begin (pretty-print (read-line out-port)) (void))           
                (loop))))))
...
(define (test-proc->llvm-error proc->llvm prog)
  (define llvm (proc->llvm prog))
  (define val0 (eval-llvm llvm))
  #t)
...
(define (racket-compile-eval e)
  (define (rewrite-match e)
    ...
    (match e [`(match ,e0 ,clauses ... (,pat0 ,es ...))
             ...
             [else e]))
  (with-handlers ([exn:fail:contract:divide-by-zero? (lambda (x) (pretty-print "Divide by 0 Exception:") (pretty-print x) (error 'eval-fail))]
                  [exn:fail:contract:arity? (lambda (x) (pretty-print "Arguement mismatch Exception:") (pretty-print x) (error 'eval-fail))]
                  [exn:fail:contract:variable (lambda (x) (pretty-print "Use of not-yet initialize variable Exception:") (pretty-print x) (error 'eval-fail))]
...
(define (racket-proc-eval procs)
  (with-handlers ([exn:fail:contract:divide-by-zero? (lambda (x) (pretty-print "Divide by 0 Exception:") (pretty-print x) (error 'eval-fail))]
                  [exn:fail:contract:arity? (lambda (x) (pretty-print "Arguement mismatch Exception:") (pretty-print x) (error 'eval-fail))]
                  [exn:fail:contract:variable (lambda (x) (pretty-print "Use of not-yet initialize variable Exception:") (pretty-print x) (error 'eval-fail))]
                  [exn:fail? (lambda (x) (begin (pretty-print "Evaluation failed:") (pretty-print x) (pretty-print procs) (error 'eval-fail)))])
                 (parameterize ([current-namespace (make-base-namespace)])
                               (namespace-require 'rnrs)
                               (namespace-require 'racket)
                               (namespace-require 'srfi/34)
