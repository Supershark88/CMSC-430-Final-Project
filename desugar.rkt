...
; Checked for the divide prim and if it has any 0s return a prim halt with the error message
(define (t-prim e env)
  (if prim-divide-by-zero-bool
      (match e
        [`(,xprim ,op ,es ...)
         (define zero (foldr (lambda (x acc) (if (equal? x '(quote 0)) #t acc)) #f es))
         (define size (length es))
         (if (and (equal? op '/) zero)
             (if (and (not (equal? 1 size)) (equal? (car es) '(quote 0)))
                 `(prim ,op . ,(map (lambda (e) (t-desugar e env)) es))
                 `(prim halt "Divide by 0 error"))
             `(prim ,op . ,(map (lambda (e) (t-desugar e env)) es)))])
      (match e
         [`(,xprim ,op ,es ...)
          `(prim ,op . ,(map (lambda (e) (t-desugar e env)) es))])))
